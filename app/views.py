...
def save_info(request):
    if request.method == 'POST':
        ...
        Register(fname=fname, lname=lname, Phone1=Phone1, email=email, address=address,
                 city=city, pin=pin, gender=gender, course=course, year=year, branch=branch).save()
...
def edit_info(request):
    if request.method == 'POST':
        ...
        course = request.POST['course']
        year = request.POST['year']
        Register.objects.filter(id=ID).update(fname=fname, lname=lname, Phone1=Phone1, email=email,
                                              gender=gender, address=address, city=city, pin=pin, branch=branch, course=course, year=year)
